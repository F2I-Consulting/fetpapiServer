			...
			if (subscription.second.context.uri == "eml:///") {
				for (const auto& oneCreated : created) {
					...
					if (filter.empty() ||
						...
						msg.change.dataObject = ETP_NS::FesapiHelpers::buildEtpDataObjectFromEnergisticsObject(oneCreated.second, subscription.second.includeObjectData);
			...
			if (subscription.second.context.uri == "eml:///") {
				for (const auto& oneUpdated : updated) {
					...
					if (filter.empty() ||
						...
						msg.change.dataObject = ETP_NS::FesapiHelpers::buildEtpDataObjectFromEnergisticsObject(oneUpdated.second, subscription.second.includeObjectData);
						session->send(msg, subscription.first);
					}
				}
			}
